...
class BcgFluctuationsExplainer(KernelExplainer):
    """
    TODO: - Write doc
    ...
    def calc_base_value(self):
        pass

    def __init__(self, model, data, x_b=None, feature_names=None, link="identity", **kwargs):
        ...
        # find E_x[f(x)] this part is overwritten
        # if x_b is provided, then the expected value
        # is calculated differently
        if self.x_b is not None:
            # SHAP calculates expected value on its own
            model_null = match_model_to_data(self.model, self.x_b)
            ...
            # self.fnull is not an array,
            # I make sure it is of the same type and shape
            self.fnull = model_null
            if isinstance(model_null, np.ndarray):
                if model_null.size == 1:
                    self.fnull = np.float64(model_null.item())
